  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/recurring", {
        ...
        body: JSON.stringify({ walletId, categoryId: categoryId || null, amount, type, description: description.trim() || null, frequency, nextDate, endDate: endDate || null }),
      })
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4 sm:space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-base sm:text-lg font-bold text-foreground">Tambah Transaksi Berulang</h1></div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border/50 shadow-sm overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Mis: Bayar kos" className="w-full h-11 px-3.5 rounded-xl border border-border/70 bg-white text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary-500/15 focus:border-primary-300 transition-all" />
          </div>
